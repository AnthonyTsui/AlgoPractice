...
#This will continously generate a new dictionary, using our previouso found inputs (starting with 0) and checking to see if possible combinations are possible
def possibleSums(coins, quantity):
    dict = {0}
    for c, q in zip(coins, quantity):
        dict = {x+c*i for x in dict for i in range(q+1)}
    return len(dict)-1